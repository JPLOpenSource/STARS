...
// For the simple object registry provided with the framework, this specifies how many objects the registry will store.
 ...
 #define FW_OBJ_SIMPLE_REG_ENTRIES           2000  //!< Number of objects stored in simple object registry
 #endif
...
// Specifies the maximum size of a byte array in a command argument
#ifndef FW_CMD_BYTES_MAX_SIZE
#define FW_CMD_BYTES_MAX_SIZE           80   //!< Max bytes size of command byte array arguments
#endif
...
#define FW_LOG_STRING_MAX_SIZE           125   //!< Max size of log string parameter type
#endif
...
enum IsfTimeBase {
    ...
    TB_1PPS_TIME, //!< Indicates time is based on 1PPS time update
    TB_DONT_CARE = 0xFFFF //!< Don't care value for sequences. If below storage type is changed, value should be changed
};
